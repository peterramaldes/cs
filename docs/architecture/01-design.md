...
3. Send invoices to customers automatically when the cargo reachs some point in its handling

## Applications

### Tracking Query

A *Tracking Query* that can access past and present handling of a particula *Cargo*
...
Two identical crates must be distinguishable, so *Cargo* objects are ENTITIES. In pratice, all shipping companies assign tracking IDs to each piece of cargo. This ID will be automatically generated, visible to the user, and in this case, probably conveyed to the customer at booking time.
...
This is a tricky one. *Delivery Histories* are not interchangeable, so, they are ENTITITES. But a *Delivery History* has one-to-one relationship with its *Cargo*, so it doesn´t really have an identitiy of its own. Its identity is borrowed from the *Cargo* that owns it. This will become clearer when we model the AGGREGATES.
